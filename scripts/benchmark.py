...
from fast_vollib import fast_black_scholes, fast_implied_volatility, get_all_greeks
...
def main() -> None:
    ...
    for _ in range(3):
        prices_warm = fast_black_scholes(flag, s, k, t, r, sigma, return_as="numpy")
        _ = fast_implied_volatility(prices_warm, s, k, t, r, flag, return_as="numpy")
    ...
    start = time.perf_counter()
    ivs = fast_implied_volatility(prices, s, k, t, r, flag, return_as="numpy")
    _sync()
    ...
    prices_np = fast_black_scholes(
        flag2, s2, k2, t2, r2, sigma2, backend="numpy", return_as="numpy"
    )
    start = time.perf_counter()
    fast_black_scholes(flag2, s2, k2, t2, r2, sigma2, backend="numpy", return_as="numpy")
    numpy_price_seconds = time.perf_counter() - start
    start = time.perf_counter()
    fast_implied_volatility(prices_np, s2, k2, t2, r2, flag2, backend="numpy", return_as="numpy")
    ...
    prices_l_warm = fast_black_scholes(flag_l, s_l, k_l, t_l, r_l, sigma_l, return_as="numpy")
    _sync()
    start = time.perf_counter()
    prices_l = fast_black_scholes(flag_l, s_l, k_l, t_l, r_l, sigma_l, return_as="numpy")
    _sync()
    large_price_seconds = time.perf_counter() - start
    start = time.perf_counter()
    fast_implied_volatility(prices_l, s_l, k_l, t_l, r_l, flag_l, return_as="numpy")
    _sync()
